...
원래 이 폴더는 로그만 찍는 C 소켓 서버 프로토타입이었는데, 대시보드용 REST/WebSocket까지 필요해지면서 Node.js 브릿지로 교체했습니다. 와이어 프로토콜은 기존 C 서버와 100% 동일하게 유지했기 때문에 `sensor_agent_client` 등 기존 클라이언트 코드는 수정 없이 접속 포트만 그대로 쓰면 됩니다. 예전 C 버전은 git 히스토리에서 언제든 확인 가능합니다.
